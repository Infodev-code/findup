...
export async function POST(request: NextRequest) {
  console.log('API Register: Requête reçue');
  
  try {
    const body = await request.json();
    console.log('API Register: Données reçues', body);
    
    const { name, email, password } = body;

    // Validation des champs
    if (!name || !email || !password) {
      console.log('API Register: Champs manquants');
    ...
    if (!emailRegex.test(email)) {
      console.log('API Register: Format d\'email invalide');
      return NextResponse.json(
    ...
    console.log('API Register: Connexion à MongoDB');
    await dbConnect();

    // Vérification si l'email existe déjà
    console.log('API Register: Vérification de l\'email');
    ...
    console.log('API Register: Utilisateur créé avec succès');
    return NextResponse.json(
